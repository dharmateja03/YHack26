...
export async function POST(req: Request) {
  ...
  const prs = (await db.collection(COLLECTIONS.prs).find({ state: "open" }).toArray()).slice(0, 5);
  const tickets = (await db.collection(COLLECTIONS.tickets).find({}).toArray()).slice(0, 5);
