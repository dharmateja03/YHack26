...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (msg === "name_required") return NextResponse.json({ error: "Name is required" }, { status: 400 });
    if (msg === "invalid_email") return NextResponse.json({ error: "Valid email is required" }, { status: 400 });
    if (msg === "weak_password") return NextResponse.json({ error: "Password must be at least 6 characters" }, { status: 400 });
    if (msg === "email_exists") return NextResponse.json({ error: "Account already exists for this email" }, { status: 409 });
